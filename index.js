...
const startThePresses = async () => {

  let numberOfArticles = findArticleFlag(process.argv);

  try {
    const rawNews = await getnews.reporter();
    const editedNews = await editnews.editor(rawNews);
    ...
    console.log('Opps, there was an error. The paper boy is running late today.');
  }
